...
var _types3 = require("../nano_contracts/types");
var _walletServiceStorageProxy = require("./walletServiceStorageProxy");
...
class HathorWalletServiceWallet extends _events.EventEmitter {
  ...
  async getUtxos(options = {}) {
    ...
    const mappedOptions = {
      tokenId: newOptions.token || _constants.NATIVE_TOKEN_UID,
    ...
    const data = await _walletApi.default.getTxOutputs(this, mappedOptions);
    ...
    for (const utxo of filteredUtxos) {
      const utxoInfo = {
        ...
        tx_id: utxo.txId,
        locked: false,
        // Wallet service UTXOs are typically available
        index: utxo.index
      };
   ...
   */
  async getUtxosForAmount(totalAmount, options = {}) {
    const newOptions = {
  ...
  async sendManyOutputsSendTransaction(outputs, options = {}) {
    ...
    if (await this.storage.isReadonly()) {
      throw new _errors.WalletFromXPubGuard('sendManyOutputsSendTransaction');
    }
    const newOptions = {
      inputs: [],
      changeAddress: null,
  ...
  async getPrivateKeyFromAddress(address, options = {}) {
    ...
    if (addressIndex === undefined) {
      throw new _errors.WalletError(`Address ${address} does not belong to this wallet`);
    }
  ...
  async getNanoHeaderSeqnum(address) {
    const addressInfo = await _walletApi.default.getAddressDetails(this, address.base58);
    return addressInfo.data.seqnum + 1;
  }
  async getAddressDetails(address) {
    const addressDetails = await _walletApi.default.getAddressDetails(this, address);
  ...
  async prepareCreateNewToken(name, symbol, amount, options = {}) {
    ...
    const isNFT = newOptions.isCreateNFT || newOptions.data !== null && newOptions.data.length > 0;
    ...
    const {
      utxos,
      changeAmount
    } = await this.getUtxosForAmount(deposit, {
      tokenId: _constants.NATIVE_TOKEN_UID
    });
    if (utxos.length === 0) {
    ...
    if (newOptions.data && newOptions.data.length > 0) {
      for (const dataString of newOptions.data) {
        dataOutputs.push(_helpers.default.createNFTOutput(dataString));
      }
    }
  ...
  async getAuthorityUtxo(tokenUid, authority, options = {}) {
    let authorityValue;
    if (authority === 'mint') {
      authorityValue = _constants.TOKEN_MINT_MASK;
    } else if (authority === 'melt') {
      authorityValue = _constants.TOKEN_MELT_MASK;
    } else {
  ...
  async prepareMintTokensData(token, amount, options = {}) {
    ...
    const ret = await this.getUtxosForAmount(0n, {
      tokenId: token,
      authority: _constants.TOKEN_MINT_MASK
    });
   ...
   */
  async getFullTxByIdForNanoContract(txId) {
    this.failIfWalletNotReady();
    ...
    if (data.tx && data.tx.version === _constants.DEFAULT_TX_VERSION) {
   ...
   */
  async prepareNanoSendTransactionAdapter(nc, pin, storage) {
    await _transaction.default.signTransaction(nc, storage, pin);
    nc.prepareToSend();
    return new _sendTransactionWalletService.default(this, {
      transaction: nc,
      pin
    });
  }
  ...
  async createNanoContractTransaction(method, address, data, options = {}) {
    ...
    if (!pin) {
      throw new _errors.PinRequiredError('Pin is required.');
    }
    ...
    if (addressIndex === undefined) {
      throw new Error(`Address used to sign the transaction (${address}) does not belong to the wallet.`);
    }

    // Get the address public key
    const addressPrivKey = await this.getAddressPrivKey(pin, addressIndex);
    const callerAddress = new _address2.default(addressPrivKey.publicKey.toAddress(this.network.getNetwork()).toString(), {
      network: this.network
    });
    ...
    const processedArgs = args.map(arg => {
      ...
      if (arg && typeof arg === 'object' && arg !== null && 'data' in arg && 'type' in arg && arg.type === 'Buffer' && Array.isArray(arg.data)) {
        // Convert back to a real Buffer
        return Buffer.from(arg.data);
      }
      return arg;
    });
    const builder = new _builder.default().setMethod(method).setWallet(wrappedWallet).setBlueprintId(data.blueprintId).setNcId(data.ncId).setCaller(callerAddress).setActions(actions).setArgs(processedArgs).setVertexType(_types3.NanoContractVertexType.TRANSACTION);
   ...
   */
  async prepareNanoSendTransactionWalletService(tx, address, pinCode, storageProxy) {
    ...
    if (!finalStorageProxy) {
      const storageProxyHelper = new _walletServiceStorageProxy.WalletServiceStorageProxy(this, this.storage);
      finalStorageProxy = storageProxyHelper.createProxy();
    }
    await _transaction.default.signTransaction(tx, finalStorageProxy, pinCode);
  ...
  async createNanoContractCreateTokenTransaction(method, address, data, createTokenOptions = {}, options = {}) {
    ...
    const mergedCreateTokenOptions = {
    ...
    const builder = new _builder.default().setMethod(method).setWallet(wrappedWallet).setBlueprintId(data.blueprintId).setNcId(data.ncId).setCaller(callerAddress).setActions(actions).setArgs(processedArgs).setVertexType(_types3.NanoContractVertexType.CREATE_TOKEN_TRANSACTION, mergedCreateTokenOptions);
    const tx = await builder.build();
