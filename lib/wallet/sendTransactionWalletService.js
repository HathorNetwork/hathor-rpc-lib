...
class SendTransactionWalletService extends _events.EventEmitter {
  constructor(wallet, options = {}) {
    ...
    _defineProperty(this, "_utxosAddressPath", void 0);
    const newOptions = {
      outputs: [],
      inputs: [],
      changeAddress: null,
  ...
  async prepareTxData() {
    ...
    for (const output of this.outputs) {
      const token = output.type === _types.OutputType.DATA ? _constants.NATIVE_TOKEN_UID : output.token;
      const value = output.type === _types.OutputType.DATA ? 1n : output.value;
    ...
    for (const [token, needsInputs] of tokenNeedsInputs.entries()) {
      if (needsInputs) {
        const {
          utxos,
          changeAmount
        } = await this.wallet.getUtxosForAmount(tokenAmountMap[token], {
          tokenId: token
        });
        if (utxos.length === 0) {
        ...
        if (!(token in userInputAmountMap)) {
          ...
          throw new _errors.SendTxError(`Invalid input selection. Token ${token} is in the outputs but there are no inputs for it.`);
        }
        if (userInputAmountMap[token] < tokenAmountMap[token]) {
          throw new _errors.SendTxError(`Invalid input selection. Sum of inputs for token ${token} is smaller than the sum of outputs.`);
        }
        if (userInputAmountMap[token] > tokenAmountMap[token]) {
    ...
    if (finalOutputs.length > this.outputs.length) {
      this.outputs = (0, _lodash.shuffle)(finalOutputs);
    } else {
    ...
    for (const input of this.inputs) {
      ...
      if (!utxo) {
        ...
        throw new _errors.UtxoError(`Could not retrieve utxo details for input ${input.txId}:${input.index}`);
      }
      dataInputs.push({
        txId: input.txId,
        index: input.index,
    ...
    for (const output of this.outputs) {
      if (output.type === _types.OutputType.DATA) {
        ...
        if (!output.address || !output.token) {
          ...
          throw new _errors.WalletError('Output is missing address or token.');
        }
        const outputType = _helpers.default.getOutputTypeFromAddress(output.address, this.wallet.network);
        dataOutputs.push({
          type: outputType,
          value: output.value,
          token: output.token,
